...
export default function ChatScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToMessages(conversationId, currentUserId, m => setMsgs(prev => [...prev, m]));
    return unsub;
  }, []);

  const load = async () => {
    const r = await getMessages(conversationId, currentUserId);
  ...
  const handleSend = async () => {
    ...
    const r = await sendMessage({ conversationId, senderId: currentUserId, content: t2, type: 'text' });
    if (!r.success) Alert.alert('Error', r.error || 'No se pudo enviar el mensaje cifrado.');
  };
